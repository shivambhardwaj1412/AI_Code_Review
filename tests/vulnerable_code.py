"""
vulnerable_code.py
Intentionally vulnerable code — for testing Security, Performance, and Style agents.
"""
import os
import sqlite3
import hashlib
...
def get_user(username):
    # BAD: SQL Injection via string concat (OWASP A03)
...
def run_report(report_name):
    # BAD: Command Injection via os.system (OWASP A01)
...
def get_all_posts():
    # BAD: N+1 query — separate DB call per post inside loop
...
def X(a, b, c, d, e, f, g):
    # BAD: Single letter name, no type hints, bare except
    try:
...
class userManager:
    # BAD: Not PascalCase class name, mutable default argument
    def __init__(self, data=[]):
        self.data = data

    def processAll(self):
        # BAD: camelCase method, no docstring
